...
def query(url, type, id, start=datetime.datetime.now().timestamp()):
    ...
    """
    response = requests.get(f'{url}/api/v1/query?query=probe_http_redirects%7B{type}="{id}"%7D&time={str(start)}')
...
def get_probe_http_redirects_user(url, id, start=datetime.datetime.now().timestamp()):
